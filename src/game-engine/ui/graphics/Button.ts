...
export default class Button implements DisplayObj {
  ...
  protected initGraphics(style?: Style): void {
    if (this.options.name) {
      this.getContainer().name = this.options.name;
    }
  ...
  protected initLabel(style?: Style): void {
    ...
    this.getContainer().removeChild(
      this.getContainer().getChildByName(Button.LABEL_NAME)
    );
  ...
  protected initShape(style?: Style): void {
    ...
    this.getContainer().removeChild(
      this.getContainer().getChildByName(Button.SHAPE_NAME)
    );
    ...
    if (style.background?.color) {
      border.beginFill(NumberUtil.parseHex(style.background?.color as string));
    }
    ...
    this.getContainer().addChildAt(border, 0);
  }

  protected initListeners(): void {
    const evt = new DetectEvent(this.getContainer());
    evt.onTap((e) => {
      // button is tapped, trigger event
      if (!this.disabled() && this.options.onTap) {
        this.options.onTap();
      }
    });

    evt.onTapDown((e) => {
      if (!this.disabled()) this.initGraphics(this.options.onClickStyle);
    });
    evt.onTapUp((e) => {
      if (!this.disabled()) this.initGraphics(this.options.style);
    });
  }

  protected width(style: Style): number {
    let value = 0;
    const text = this.getContainer().getChildByName(Button.LABEL_NAME);
    if (text) {
  ...
  protected initOptions(): void {
    ...
    this.options.type = this.originalOptions.type;
    ...
    if (this.options.type === "normal") {
      ...
      this.options.style = this.createStyle([
        this.originalOptions.style,
        this.isDisabled() ? CONFIG.text.disabledStyle : undefined,
        CONFIG.text.defaultStyle,
      ]);
      this.options.onClickStyle = this.createStyle([
        this.originalOptions.onClickStyle,
        this.isDisabled() ? CONFIG.text.disabledStyle : undefined,
        CONFIG.text.onClickStyle,
        CONFIG.text.defaultStyle,
      ]);
    } else {
      throw Error(`Type is invalid ${this.options.type}`);
    }
  }
